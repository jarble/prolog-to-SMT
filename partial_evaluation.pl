...
find_all_clauses(Var,Var) :-
	\+callable(Var);var(Var);is_list(Var);matches_any([length(_,_),_>_,_<_,_=_,_==_,_\=_,_\==_,_=<_,_=<_,_**_,dif(_,_),_ is _,false,true,max_list(_,_),min_list(_,_),union(_,_,_),intersection(_,_,_),subset(_,_),all_different(_),all_distinct(_),nth0(_,_,_),nth1(_,_,_)],Var).
